...
def obtener_info_gif(ruta_gif):
    try:
        with open(ruta_gif, 'rb') as file:
            header = file.read(6)  # Leer los primeros 6 bytes que contienen el encabezado GIF
            version = header.decode("utf-8")
        ...
        with Image.open(ruta_gif) as img:
            if isinstance(img, GifImagePlugin.GifImageFile):
                ...
                # Formato numérico (en este caso usamos bits por pixel, se encuentra en la paleta)
                bits_per_pixel = img.tile[0][1][3] if img.tile else "Desconocido"
                ...
                return {
                    ...
                    "Formato numérico (Bits por pixel)": bits_per_pixel
                }
